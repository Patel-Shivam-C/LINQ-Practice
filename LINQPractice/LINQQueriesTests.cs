...
public class LINQQueriesTests
{
    ...
    {
        // Act
        var result = (from p in people where p.Age >= 35 && p.Age <= 40 && p.Hobbies.Contains("Reading") select p.Name).ToList();
    ...
    {
        // Act
        var vowel = new List<String>() { "a", "e", "i", "o", "u" };
        var result = people.Any(p => vowel.All(p.Name.Contains));
    ...
    {
        // Act
        var result = (from p in people where p.Name.StartsWith("A") && p.Age < 30 select p).ToList();
    ...
    {
        // Act
        var result = people.OrderBy(p => p.Age).LastOrDefault();
    ...
    {
        // Act
        var result = people.Any(p => p.Hobbies.Any(hob => hob.StartsWith("G")));
    ...
    {
        // Act
        var result = people.Count(p => p.Hobbies.Any(hob => hob.StartsWith("S")));
